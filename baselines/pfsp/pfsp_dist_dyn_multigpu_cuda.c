  ...
  *D = 1;
  ...
  {
  printf("\n=================================================\n");
  printf("Distributed multi-GPU C+CUDA+OpenMP+MPI %d thread(s) %d GPU(s)\n\n", numLocales, D);
  ...
  void print_results_file(const int inst, const int machines, const int jobs, const int lb, const int D, const int numLocales, const int optimum,
  ...
  fprintf(file,"ta%d lb%d %dthreads %dGPU %.4f %llu %llu %d\n",inst,lb,numLocales,D,timer,exploredTree,exploredSol,optimum);
  fclose(file);
  return;
  }
  ...
  *a = tmp;
  ...
  for (int i = parent.limit1+1; i < jobs; i++) {
     ...
     double* elapsedTime, int locID, int numLocales)
   {
    ...
    fill_johnson_schedules(lbound1->p_times, lbound2);
      ...
      // For now, we develop the initial tree throughout all MPI processes
      /*
    ...
    {  
      ...
      // MPI_Barrier(MPI_COMM_WORLD); (comment because of reduce above)
    ...
    int eachLocaleBest = *best;
      ...
      _Atomic bool eachLocaleState = false; // one locale per compute node
      // WARNING: ATTENTION WITH THIS VARIABLE (LOOK CHAPEL)
      _Atomic bool allLocalesIdleFlag = false;

      // For GPUs under each MPI thread
    ...
    #pragma omp parallel for num_threads(D) shared(eachExploredTree, eachExploredSol, eachBest, eachTaskState, globalTerminationFlag, eachLocaleState, pool_lloc, multiPool, lbound1, lbound2) //reduction(min:best_l)
      for (int gpuID = 0; gpuID < D; gpuID++) {
    	...
    	Each task gets its parenst nodes from the pool
          */
            ...
            if (poolSize > 0) {
              ...
              if (locState == true){
               locState = false;
               atomic_store(&eachLocaleState,false);
             }
    	...
    	*/
            ...
            // local work stealing
             int tries = 0;
             bool localSteal = false;
             bool globalSteal = false;

    	// local work stealing attempts
             int victimTasks[D];
             permute(victimTasks,D);

            while (tries < D && localSteal == false) { //WS0 loop
              const int victimTaskID = victimTasks[tries];

              if (victimTaskID != gpuID) { // if not me
                SinglePool_ext* victim;
                victim = &multiPool[victimTaskID];
                nSteal ++;
                int nn = 0;
  ...
  if(gpuID == 0){
    ...
    if (allIdle(allLocaleState, numLocales, &allLocalesIdleFlag)) {
      ...
      free(allLocaleState);
        // Set a global flag for termination
  ...
  deleteSinglePool_ext(pool_loc);
      ...
      printf("Are we out of the GPU/OpenMP step? locID = %d \n",locID);
      ...
      *******************************/
      ...
      if (locID == 0) {
        ...
        allEachExploredTrees = (unsigned long long int *)malloc(numLocales * D * sizeof(unsigned long long int));
      }
  ...
  if (locID == 0) {
    for(int i = 0; i < totalSize; i++)
      ...
      //Checking info after pushBack on each thread
      //printf("thread[%d] After pushBack pool.size= %d\n",locID, pool.size);
  ...
  if (locID == 0){
    ...
    while (1) {
      ...
      count++;
        ...
        //printf("After decompose Size of the explored tree: %llu\n", *exploredTree);
        //printf("After decompose Number of explored solutions: %llu\n", *exploredSol);
        //printf("thread[%d] Step 3 after break while count = %d\n",locID, count);
  }

      // freeing memory for structs common to all MPI threads
  ...
  if (rank == 0)
    print_settings(inst, machines, jobs, ub, lb, D, size);
