module pfsp_search_distributed
{
  ...
  *******************************************************************************/
  ...
  // Evaluate and generate children nodes on CPU.
  proc decompose(const jobs, const lb1_data, const lb2_data,
  ...
  proc evaluate_gpu_lb1(const parents_d: [] Node, const size, const jobs, const lbound1_d, ref bounds_d)
  {
    @assertOnGpu
  ...
  */
  ...
  proc evaluate_gpu_lb1_d(const parents_d: [] Node, const size, const best, const jobs, const lbound1_d, ref bounds_d)
  {
    @assertOnGpu
  ...
  proc evaluate_gpu_lb2(const parents_d: [] Node, const size, const best, const jobs, const lbound1_d, const lbound2_d, ref bounds_d)
  {
    @assertOnGpu
    foreach threadId in 0..#size {
      const parentId = threadId / jobs;
  ...
  proc evaluate_gpu(const parents_d: [] Node, const size, const best, const jobs, const lbound1_d, const lbound2_d, ref bounds_d)
  {
    select lb {
      when "lb1_d" {
          ...
          if (poolSize > 0) {
            local {
              ...
              */
              ...
              on device do evaluate_gpu(parents_d, numBounds, best_l, jobs, lbound1_d, lbound2_d, bounds_d); // GPU kernel
              bounds = bounds_d; // device-to-host

              /*
                Each task generates and inserts its children nodes to the pool.
              */
              generate_children(jobs, parents, poolSize, bounds, tree, sol, best_l, pool_loc);
            }
          }
          else {
